...
class Repository:
    ...
    async def list_candidates(self, limit: int = 3) -> list[Any]:
        rows = await self.conn.fetch(
            ...
            WHERE rc.state IN ('eligible','watched','rejected')
    ...
    async def decide_candidate(
        ...
        candidate = await self.conn.fetchrow(
            """SELECT rc.id, rc.strategy_version_id,
        ...
        row = await self.conn.fetchrow(
            ...
            (idempotency_key,candidate_id,strategy_version_id,git_commit_sha,rank,operator_action,operator_note,decided_at)
            VALUES ($1,$2,$3,$4,$5,$6,$7,now()) ON CONFLICT (candidate_id) DO UPDATE SET
        ...
        for kind, interval in (
            ...
            await self.conn.execute(
                """INSERT INTO outcome_checks (idempotency_key,candidate_id,check_kind,due_at,status,outcome_method)
                VALUES ($1,$2,$3,now()+$4::interval,'pending','dexscreener_market_price')

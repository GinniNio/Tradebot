...
class MarketWatchService:
    ...
    async def _persist_pair(self, pair: dict[str, Any]) -> None:
        ...
        event_key = hashlib.sha256(
            f"dexscreener:{pair_address}:{snap['captured_at'].isoformat()}".encode()
        ).hexdigest()
        async with self.db.acquire() as conn, conn.transaction():
            strategy_id = await self._strategy_id(conn)
            recent = await conn.fetchval(
                """SELECT id FROM research_candidates WHERE token_address=$1 AND selected_pair=$2
                    AND strategy_version_id=$3 AND created_at > now()-interval '24 hours' LIMIT 1""",
                token,
                pair_address,
                strategy_id,
            )
